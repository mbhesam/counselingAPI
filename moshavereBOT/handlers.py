from moshavereBOT.services import get_question_answers, check_information, get_question_prefix,\
    get_questions_name, get_question_text, get_next_question_id, get_state_id, set_state_id
...
async def start_handler(update: Update):
    ...
    try:
        ...
        pass
    question_text = get_question_text(question_id=0)
    question_name: str = get_questions_name(question_id=0)[0]
    ...
    set_state_id(user_name=update.effective_user.username, state_id=0)
    return STATES[question_name]


async def switch_handler(update: Update):
